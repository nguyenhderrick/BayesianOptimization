...
BayesianOptimization.MySQL$methods(
  ...
  fit = function(nug.thres = 20, ...){
    ...
    xgbout <- dReadTable(con, "XGBOUT")
    xgb.x <- xgbout[names(hyper.bounds)]
    scaled.xgb.x <- mapply(function(x, y) rescale(x, from = y), xgb.x, bounds)
    .self$gaussian.process <- 
      GP_fit(scaled.xgb.x, xgbout$test.auc.mean, nug_thres = nug.thres,...)
  },
